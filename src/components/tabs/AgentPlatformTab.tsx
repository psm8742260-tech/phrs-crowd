  ...
  const handleStudioChatSubmit = async (text: string) => {
    ...
    try {
      ...
      if (data.success && data.text) {
        setAgentChatHistory((prev: any) => [...prev, { role: 'model', text: data.text }]);
      } else {
        setAgentChatHistory((prev: any) => [...prev, { role: 'model', text: `⚠️ Error: ${data.error || 'Server responded with an error.'}` }]);
      }
    } catch (err: any) {
      setAgentChatHistory((prev: any) => [...prev, { role: 'model', text: `⚠️ Connection Error: ${err.message || 'Failed to reach API server.'}` }]);
    } finally {
                    ...
                    <div className="md:col-span-8 flex flex-col h-[400px] border border-slate-200 rounded-xl bg-slate-50/20">
                      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
                        ...
                        {isAgentThinking && (
                          <div className="flex justify-start">
                            <div className="max-w-[80%] p-3 rounded-2xl text-xs bg-white border border-slate-200 text-slate-500 rounded-tl-none flex items-center gap-2">
                              ...
                              <span className="italic text-slate-400">ఆలోచిస్తోంది...</span>
                            </div>
                          </div>
                        )}
                      </div>
                      <div className="p-3 border-t border-slate-100 bg-white rounded-b-xl flex gap-2">
                        <input 
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter' && agentChatInput.trim() && !isAgentThinking) {
                              handleStudioChatSubmit(agentChatInput);
                            }
                          }}
                          disabled={isAgentThinking}
                          className="flex-1 px-4 py-2 text-xs rounded-full bg-slate-100 border-none focus:ring-1 focus:ring-blue-500 disabled:opacity-60"
                        />
                        <button 
                          onClick={() => {
                            if (agentChatInput.trim() && !isAgentThinking) {
                              handleStudioChatSubmit(agentChatInput);
                            }
                          }}
                          disabled={isAgentThinking}
                          className="p-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition disabled:bg-slate-400"
                        >
